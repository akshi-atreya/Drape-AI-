...
function deriveTrendTags(color, subcategory, material) {
  ...
  if (["Blazer", "Wide-Leg Trousers", "Tailored Pants", "Wool Coat"].includes(subcategory)) tags.push("Relaxed Tailoring");
  if (["Jewelry Set", "Wide-Brim Hat", "Belt", "Sunglasses"].includes(subcategory)) tags.push("Statement Accessories");
  if (subcategory === "Ballet Flats") tags.push("Ballet Flats");
  if (["Turtleneck", "Wool Coat", "Knit Top"].includes(subcategory)) tags.push("Layering");
  if (["Suede Jacket", "Ankle Boots", "Knee-High Boots"].includes(subcategory) && chance(0.4)) tags.push("Western-Inspired");
  if (["Aritzia - Wilfred", "Nordstrom - Halogen"].length && chance(0.15)) tags.push("Quiet Luxury");
...
for (const [category, count] of Object.entries(TARGET_PER_CATEGORY)) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const silhouette = pick(def.silhouette ? def.silhouette : def.silhouettes);
